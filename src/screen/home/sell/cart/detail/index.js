...
import { Text, View, TouchableOpacity, TextInput, Switch, ScrollView, FlatList } from 'react-native'
...
export default class ItemCartDetail extends Component {
    ...
    componentWillMount() {
        console.log('item chi tiet: ' + JSON.stringify(this.props.item))
    }
        ...
        if (itemChange != '-1') {
            if (type == '-') {
                if (parseFloat(Sell.listCart[itemChange].value + '') > parseFloat(Sell.listCart[itemChange].valueDefault + '')) {
                    ...
                    if (Sell.listCart && Sell.listCart.length > 0) {
                    } else {
                        Sell.deleteCart()
                        Navigation.popTo('SellScreen')
                    }
    ...
    render() {
        ...
        return (
            <View style={{ width: '100%', flex: 1, justifyContent: 'center', alignItems: 'center' }}>
                ...
                {/* <ButtonGradient
                    onPress={this.goBackCart}
                    containStyle={{ width: '80%', marginBottom: 10 + values.bottomIphoneX }}
                    title={'Lưu'}
                /> */}
                <ExtraInfoView
