...
class Setting {
  ...
  updatePrinter(id, model, name, status, auto) {
    ...
    if (indexItem + "" != "-1") {
      this.listPrinter[indexItem].model =
        model != undefined ? model : this.listPrinter[indexItem].model;
      this.listPrinter[indexItem].name =
        name != undefined ? name : this.listPrinter[indexItem].name;
      this.listPrinter[indexItem].status =
        status != undefined ? status : this.listPrinter[indexItem].status;
      this.listPrinter[indexItem].auto =
        auto != undefined ? auto : this.listPrinter[indexItem].auto;
    }
  ...
  getListPrinter(callback = null) {
    ...
    BluetoothSerial.on("connectionLost", () => {
      if (this.devices) {
        SimpleToast.show(
          `Connection to device ${this.devices.name} has been lost`
        );
      }
    ...
    BluetoothSerial.pairDevice(device.id)
      .then(paired => {
        if (paired) {
          ...
          devices.push(device);
          this.devices = devices;
          this.unpairedDevices = this.state.unpairedDevices.filter(
            d => d.id !== device.id
          );
        } else {
